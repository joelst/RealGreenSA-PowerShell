...
.OUTPUTS
    Hashtable
#>
function Get-RgSaHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-RgSaConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-RgSaStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-RgSaUnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-RgSaUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-RgSaStringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-RgSaKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-RgSaRSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-RgSaECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Deprecated
    <#$cryptographicScheme = Get-RgSaCryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
function Get-RgSaRSASignature {
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "RgSaRSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-RgSaECDSASignature {
    param(
...
function Get-RgSaCryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-RgSaKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
